              ...
              {historySessions.length === 0 ? (
                 ...
                 historySessions.map((session, idx) => (
                   <button 
                     ...
                     className={`w-full py-1 px-5 rounded-[1.5rem] flex items-center gap-4 transition-all text-left border-2 ${
                   ...
                   >
                     <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors ${
                        session.sessionId === currentSessionId ? 'bg-pharma-green/20 text-pharma-green' : 'bg-slate-50 text-slate-300'
                     }`}>
                        <MessageSquare size={16} strokeWidth={2.5} />
                     </div>
                     <div className="flex-1 overflow-hidden">
                        <p className={`text-[13px] font-black truncate leading-tight transition-colors ${
                           session.sessionId === currentSessionId ? 'text-pharma-green' : 'text-slate-700'
                        }`}>
                          {session.title}
                        </p>
              ...
              {messages.map((m, i) => (
                ...
                >
                  <div className={`flex gap-5 max-w-[90%] lg:max-w-[75%] ${m.role === "user" ? "flex-row-reverse" : ""}`}>
                    <div className={`w-12 h-12 rounded-[1.25rem] flex items-center justify-center flex-shrink-0 shadow-[0_12px_24px_-8px_rgba(0,0,0,0.15)] mt-1 ${
                      m.role === "user" ? "bg-slate-950 text-white" : "bg-white border border-slate-100 text-pharma-green"
                    }`}>
                      {m.role === "user" ? <User size={22} /> : <Bot size={22} />}
                    </div>
                    <div className={`px-8 py-6 rounded-[2.5rem] text-[16px] font-black leading-relaxed shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)] relative ${
                        m.role === "user"
            ...
            {loading && (
              <div className="flex justify-start py-10">
                <div className="flex gap-5">
                  ...
                  <div className="px-10 py-6 rounded-[2.5rem] rounded-tl-none bg-white border border-slate-50 flex items-center gap-2.5 shadow-xl shadow-slate-200/40">
