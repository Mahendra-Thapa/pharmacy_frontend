    ...
    return (
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {addresses.map(addr => (
                    <Card key={addr.id} className={`group relative rounded-xl border-2 p-8 transition-all duration-500 overflow-hidden ${addr.default ? 'border-pharma-blue bg-white shadow-2xl shadow-pharma-blue/10' : 'border-slate-50 bg-white hover:border-slate-200 shadow-sm'}`}>
